		...
		<div class="panel-body">	
			...
			<div class="col-md-7">	
					<table class="table table-hover table-responsive table-bordered">
							<tr>
								<th width=50>No</th>
								<th width=70>Id Hak Akses</th>
								<th width=100>Nama Hak Akses</th>
								<th width=50>Aksi</th>
							</tr>
							<?php $no=0; foreach($akses_tu as $row): $no++;?>
							<tr>
								<td><?php echo $no;?></td>
								<td><?php echo $row->id_hak_akses;?></td>
								<td><?php echo $row->nama_akses;?></td>
								<td align='center'>
										<a onclick="return confirm('Anda Yakin?');" href="<?php echo site_url('tata_usaha/hapusakses/'.$id.'/'.$row->id_hak_akses);?>" class='tooltipsku' data-toggle='tooltip' data-placement='top' title='hapusakses'>
							...
							<tr>
								<th colspan=3>
									<form method='post' action="<?php echo site_url('tata_usaha/tambahakses');?>">
										<select required name="akses" class="form-control">
											<option value="">Pilih Hak Akses</option>
											<?php $no=0; foreach($akses as $row): $no++;?>
											<option value="<?php echo $row->id_hak_akses;?>"> <?php echo $row->nama_akses;?> </option>
											<?php endforeach; ?>
											<input type='hidden' name='id_tu' value="<?php echo $id?>">
										</select>
								</th>
								<td align='center'>
										<button type='submit' class='btn btn-success' aria-label='Left Align'><span class="glyphicon glyphicon-ok-sign" aria-hidden="true"></span> Tambah</button>
									</form>
								</td>
							</tr>
					</table>
